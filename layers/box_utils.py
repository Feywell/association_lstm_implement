...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
   ...
   # print('before encode matches: ',matches[0])
   # print('before encode priors: ',priors[0])
    loc = encode(matches, priors, variances)
   # print('after encode loc: ',loc[0])
    loc_t[idx] = loc    # [num_priors,4] encoded offsets to learn
    conf_t[idx] = conf  # [num_priors] top class label for each prior
   # print('after encode loc_t ',loc_t[0])
...
def nms(boxes, scores, overlap=0.5, top_k=200):
   ...
   # w = boxes.new()
   # h = boxes.new()
    #print('the first w size: ',w.size())
    #print('the first h size: ',h.size())
    ...
    while idx.numel() > 0:
        ...
        # idx = Variable(idx)  # add for autograde this version
