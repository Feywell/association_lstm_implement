...
class VIDAnnotationTransform(object):
...
class VIDetection(data.Dataset):
    ...
    def __init__(self, root, phase='train',
                 ...
                 transform=None, target_transform=VIDAnnotationTransform(),
    ...
    def pull_item(self, index):
        ...
        # print('num:', self.num, 'img_id:', img_id)
        # print('annopath: ', self._annopath % img_id)
        ...
        if self.transform is not None:
            ...
            if target.shape == (0,):
            #     index = index -1
                  print(self._annopath % img_id)    # 可考虑凭借此滤除
            #     img_id_pre = self.ids[index]
            #
            #     target = ET.parse(self._annopath % img_id_pre).getroot()
            #     img = cv2.imread(self._imgpath % img_id_pre)
            #     height, width, channels = img.shape
            #
            #     if self.target_transform is not None:
            #         target = self.target_transform(target, width, height)
